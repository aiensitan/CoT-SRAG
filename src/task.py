...
with open("../config/config.yaml", "r", encoding="utf-8-sig") as file:
...
# ======================
# Offline token length estimator (NO HF)
# ======================
def get_word_len(text: str) -> int:
    """
    A robust offline approximation of token length.
    - English: ~1 token per 0.75 words => words * 1.3
    - Chinese: ~1 token per char (rough)
    This is ONLY used for filtering/truncation thresholds, not model inference.
    """
    if not text:
        return 0
    english_words = re.findall(r"[A-Za-z]+", text)
    chinese_chars = re.findall(r"[\u4e00-\u9fff]", text)
    # add some penalty for numbers / symbols
    digits = re.findall(r"\d+", text)
    return int(len(english_words) * 1.3 + len(chinese_chars) + len(digits) * 0.3)
...
def build_ext_instruction(model, question, answer, content, support, min_res_tokens):
    ...
    try:
        ...
        logger.info(f"{response} CONFIGURATION")
